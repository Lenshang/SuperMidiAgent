...
export function buildSystemPrompt(opts: { sessionMidis: MidiAssetMeta[]; kbContext: string }): string {
  ...
  lines.push('- 修改 MIDI 用 modify_midi（生成新版本，不覆盖原版）：增加真实力度用 humanize_velocity；增加真实表情起伏用 add_cc11；改变和弦进行用 change_chords；还有 transpose、quantize、add_sustain、set_tempo、set_program 等操作。多个操作可以放在一次调用的 operations 数组中按顺序执行。');
